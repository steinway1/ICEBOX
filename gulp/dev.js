...
gulp.task("twig-admin:dev", () => {
  return gulp
    ...
    .pipe(
      data(function () {
        return JSON.parse(
          fs.readFileSync(`src/templates/data/admin/data.json`)
        );
      })
    )
    .pipe(
      data(function (file) {
        return JSON.parse(
          fs.readFileSync(
            root.src.adminData + path.basename(file.path) + ".json"
          )
        );
      })
    )
...
gulp.task("twig:dev", () => {
  return gulp
    ...
    .pipe(
      data(function (file) {
        return JSON.parse(
          fs.readFileSync(root.src.data + path.basename(file.path) + ".json")
        );
      })
    )
    .pipe(twig({ base: "src/templates", errorLogToConsole: true }))
...
gulp.task("watch:dev", () => {
  gulp.watch(
    "./src/templates/**/*.twig",
    gulp.parallel("twig:dev", "twig-admin:dev")
  ),
    gulp.watch(
      "./src/templates/data/**",
      gulp.parallel("twig:dev", "twig-admin:dev")
    ...
    gulp.watch(
      "./src/templates/admin/assets/**/*",
      gulp.parallel("assets-admin:dev")
    ),
    gulp.watch(
      "./src/scss/**/*.scss",
      gulp.parallel("css:dev", "css-admin:dev", "css-promo:dev")
    ),
    gulp.watch(
      ["./src/js/*.js", "./src/js/modules/**/*.js"],
