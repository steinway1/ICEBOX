...
import { createElem } from "../general/utils";

import { updateInputsAllowOnlyDecimals } from "../general/init-fn";

import { formatAndSumNumbers } from "../general/utils";
...
export default class ManualOrderForm {
  constructor(rootEl, popupInstance, orderID) {
    ...
    this.customerInputArr = [
      ...document.querySelectorAll("[data-customer-input]"),
    ];
    ...
    this.customerPhotoWrap = document.querySelector(
      ".m-popup__custom-pic-wrap"
    );
    this.customerPhotoElem = document.querySelector("[data-customer-photo]");

    this.paidInFullSelect = document.querySelector("#paidInFull");
    this.todayPaymentInput = document.querySelector("#orderTodayPayment");
    ...
    this.searchCustomerContainer = document.querySelector(
      "#searchListCustomers"
    );
    this.selectedItemContainer = document.querySelector(
  ...
  init() {
    ...
    this.productSearch = new ManualOrderProductSearch(
      this.searchProductContainer,
      this
    );
    this.customerSearch = new ManualOrderCustomerSearch(
      this.searchCustomerContainer,
      this
    );
  ...
  bindPaidInFullToggle() {
    if (this.paidInFullSelect && this.todayPaymentInput) {
      this.paidInFullSelect.onchange = () => {
        ...
        if (enable) {
          this.todayPaymentInput.value = formatAndSumNumbers(
            this.#getAmountValuesArray()
          );
        } else {
          this.todayPaymentInput.value = "0";
        }
      };
    }
  }
  #getAmountValuesArray() {
    const itemsArr = [
  ...
  resetOtherDetails() {
    ...
    if (otherDetails) {
      const textInputArr = [
        ...otherDetails.querySelectorAll('input[type="text"]'),
      ];
  ...
  renderManualItem(item) {
    ...
    const createSelectFields = () => {
      ...
      const selectElementsHTML = options
        .map((option) => {
  ...
  fillCustomerInputs(customer) {
    const { customer_id, first_name, last_name, email, phone, img_src } =
      customer;
    const resetButton = createElem("div", {
      className: "blank-btn",
    ...
    if (email && !this.customerInputEmail.disabled)
      this.customerInputEmail.value = email;
  ...
  setupInitialSteps() {
    ...
    const prevStepBtn =
      this.rootEl.querySelector("[data-prev-step]") ||
      this.rootEl.querySelector("[data-manual-close]");
  ...
  goStep(step = 1) {
    ...
    if (isLastStep) {
      const totalAmountInput = this.form.querySelector("#orderTodayAmount");
      if (totalAmountInput) {
        totalAmountInput.value = formatAndSumNumbers(
          this.#getAmountValuesArray()
        );
      }
    }
  ...
  getFormData() {
    ...
    const cleanEntries = Array.from(formData).filter(
      ([key]) => ![...config.ignore, ...config.arrays].includes(key)
    );
