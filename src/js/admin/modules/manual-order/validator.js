class ItemValidator {
  ...
  validate() {
    const itemTitleInput = this.contentElem.querySelector(
      'input[name="item_title"]'
    );
    const selectArr = [
      ...this.contentElem.querySelectorAll("select[required]"),
    ];
...
class CustomerValidator {
  ...
  validate() {
    const customerInputArr = [
      ...this.contentElem.querySelectorAll(
        "input[required][data-customer-input]"
      ),
    ];
    const emailInput = this.contentElem.querySelector(
      'input[name="email"][required][data-customer-input]'
    );
    ...
    if (firstNotFilled) {
      return {
        result: false,
        msg: `Enter customer ${firstNotFilled.getAttribute("placeholder")}.`,
      };
    }
...
class OtherDetailsValidator {
  ...
  validate() {
    const requiredSelectArr = [
      ...this.contentElem.querySelectorAll("select[required]"),
    ];
    const requiredInputArr = [
      ...this.contentElem.querySelectorAll("input[required]"),
    ];
    const dateInput = this.contentElem.querySelector(
      'input[name="order_date"][required]'
    );

    if (
      requiredSelectArr.every((select) => !select.value) &&
      requiredInputArr.every((input) => !input.value)
    ) {
    ...
    if (firstNotFilled) {
      return {
        result: false,
        msg: `Enter ${firstNotFilled.getAttribute("placeholder")}`,
      };
    }

    const psRequired = [
      ...this.contentElem.querySelectorAll(
        "div[data-ps-multiple][data-ps-required]"
      ),
    ];
    if (psRequired.length) {
      const invalidPs = psRequired.find((ps) => {
        const current = ps.querySelector("[data-ps-selected]");
        return current && current.textContent.toLowerCase().includes("empty");
      });
...
export default class ManualOrderValidator {
  constructor(contentType, contentElem) {
    this.validatorStrategy = this.getValidatorStrategy(
      contentType,
      contentElem
    );
  }
