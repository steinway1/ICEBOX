...
class PriceInfoModals {
  ...
  #handleClick(e) {
    ...
    if (!modalData)
      this.#throwError(
        'No modal data',
        `No modal data found for the [data-calc-info] attribute value: <strong>${key}</strong>`,
      );

    const { heading, formula } = modalData;
    if (!heading || !formula)
      this.#throwError('No heading or formula', 'Heading or formula is missing from the PriceInfoModals.Map');
