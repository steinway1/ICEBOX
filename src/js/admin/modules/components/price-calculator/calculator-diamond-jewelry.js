...
class CalculatorDiamondJewelry {
  ...
  #calcMetalCost() {
    ...
    if (!basePriceInput) {
      this.resultDiaMetalCost.textContent = "$ 0.00";
      this.#showError(`Base price input not found for metal : ${metal}. Please check the input selector : ${basePriceSelector}`);
      return;
    }
    ...
    if (isNaN(basePrice)) {
      this.resultDiaMetalCost.textContent = "$ 0.00";
      this.#showError(`Base price input is not a number for metal : ${metal}. Please check the input value : ${basePriceInput.value}`);
      return;
    }
  ...
  #calcDiaCost() {
    ...
    if (!Number.isFinite(diamondCost) || !Number.isFinite(metalCost)) {
      this.resultDiaCost.textContent = "$ 0.00";
      this.#showError(`Please enter a valid diamond cost and metal cost. Current Diamond Cost : ${diamondCost} and Current Metal Cost : ${metalCost}`);
      return;
    }
