...
import { appendPageLoader, removePageLoader, arrayIntoChunks } from '../../general/utils';
...
export default class CatalogSwitcher {
  constructor(rootEl, catalogInstance) {
    ...
    this.catalogTitleArr = [...this.rootEl.querySelectorAll('[data-catalog-title]')];
    this.catalogItemsCountArr = [...this.rootEl.querySelectorAll('[data-catalog-items-count]')];
  ...
  async switch(collectionName, input) {
    ...
    //   const collection = await fakeAjaxGetCatalogCollection(collectionName);
      if (!collection) {
        ...
        this.list.innerHTML = new CatalogSheetsGenerator().generatePlaceholder();
